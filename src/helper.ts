...
class marketDataUpdateHelper {
    ...
    async onMarketDataUpdate(symbol: Array<string>, accessToken: string, callback: Function) {
        ...
        this.Instance = socketWrapper(url, dataString, (data: any) => {
...
const apiUrls = {
    ...
    funds: `${apiUrl}funds`,
    holdings: `${apiUrl}holdings`,
    orders: `${apiUrl}orders`,
    tradeBook: `${apiUrl}tradebook`,
    filterOrder: (id: number) => `${apiUrl}orders?id=${id}`,
    positions: `${apiUrl}positions`,
    marketStatus: `${apiUrl}market-status`,
    placeOrder: `${apiUrl}orders`,
    placeMultiOrder: `${apiUrl}orders-multi`,
    modifyOrder: `${apiUrl}orders`,
    cancelOrder: `${apiUrl}orders`,
    exitPosition: `${apiUrl}positions`,
    convertPosition: `${apiUrl}positions`,
    marketDataHistory: ({ symbol: string, resolution: string, dateFormat: number, from: string, to: string, flagCount: number }) => `${dataApi}history/?symbol=${symbol}&resolution=${resolution}&date_format=${format}&range_from=${from}&range_to=${to}&cont_flag=${flag}`,
}
